...
Приняв $\lambda_{\max} \approx 2$, $K=1$ и $\theta = \tilde{\theta}_0  = -\tilde{\theta}_1$, получаем
		...
		\end{align*}
\textit{Трюк перенормировки}: $\tilde{\mathbf{A}}=\mathbf{A}+\mathbf{I}_{n},~ \tilde{\mathbf{D}}_{i i}=\sum_{j} \tilde{\mathbf{A}}_{i j}.$

		$$\boxed{\mathbf{U}= \tilde{\mathbf{D}}^{-\frac{1}{2}} \tilde{\mathbf{A}}\tilde{\mathbf{D}}^{-\frac{1}{2}} \mathbf{X} \mathbf{W}}$$
...
\begin{frame}{Модель нейронной сети}
	\begin{figure}[h]
		...
		\caption{Структура графовой свёрточной нейронной сети, использованной в данной работе}
	\end{figure}
...
\begin{frame}{Результаты обучения на 10 эпохах}
	...
	\centering
	\includegraphics[width=0.75\textwidth]{training_correlations_smallsettt.pdf}
